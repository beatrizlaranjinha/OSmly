...
let test_memory_allocation () =
  let addr1_opt = Memory.allocate_memory [] 100 in
  assert (addr1_opt = Some 0);
  let addr2_opt = Memory.allocate_memory [(0, 100)] 50 in
  assert (addr2_opt = Some 100);
  let addr3_opt = Memory.allocate_memory [(100, 50)] 20 in
...
let test_command_E_math () =
  ...
  let pcb1 = Process.create_pcb 1 0 "prog" 0 3 1 0 None in
...
let test_command_E_block () =
  ...
  let manager = Manager.create_manager memory Manager.Priority in
  let pcb1 = Process.create_pcb 1 0 "prog" 0 1 1 0 None in
  let manager_ready = { manager with Manager.ready_queue = [pcb1] } in
  let manager_after = Manager.command_E manager_ready in
...
let test_command_E_fork () =
  ...
  let manager = Manager.create_manager memory Manager.Priority in
  let pcb1 = Process.create_pcb 1 0 "prog" 0 6 1 0 None in
  ...
  let p_pai = match manager_after.Manager.running with Some p -> p | None -> failwith "Sem pai a correr" in
  let p_filho = List.hd manager_after.Manager.ready_queue in
...
let test_command_I () =
  let memory = Memory.create_memory () in
  let manager = Manager.create_manager memory Manager.Priority in
  let pcb1 = Process.create_pcb 1 0 "p1" 0 1 1 0 None in
  let p1_running = Process.update_state pcb1 Process.Running in
  let manager_running = { manager with Manager.running = Some p1_running } in
  let manager_after = Manager.command_I manager_running in
  match manager_after.Manager.blocked_queue with
...
let test_scheduler_priority () =
  let memory = Memory.create_memory () in
  let manager = Manager.create_manager memory Manager.Priority in
  let pcb_low = Process.create_pcb 1 0 "p1" 0 1 5 0 None in
  let pcb_high = Process.create_pcb 2 0 "p2" 0 1 1 0 None in
...
let test_scheduler_fcfs () =
  ...
  let pcb2 = Process.create_pcb 2 0 "p2" 0 1 1 10 None in
  let pcb1 = Process.create_pcb 1 0 "p1" 0 1 1 5 None in
...
let test_scheduler_sjf () =
  ...
  let pcb_large = Process.create_pcb 1 0 "p1" 0 50 1 0 None in
  let pcb_small = Process.create_pcb 2 0 "p2" 0 10 1 0 None in
...
let test_scheduler_edf () =
  ...
  let pcb1 = Process.create_pcb 1 0 "p1" 0 10 10 0 None in
  let pcb2 = Process.create_pcb 2 0 "p2" 0 10 2 5 None in
...
let test_scheduler_rm () =
  let memory = Memory.create_memory () in
  let manager = Manager.create_manager memory Manager.RM in
  let pcb1 = Process.create_pcb 1 0 "p1" 0 10 10 0 (Some 20) in
  let pcb2 = Process.create_pcb 2 0 "p2" 0 10 2 0 (Some 15) in
  let manager_ready = { manager with Manager.ready_queue = [pcb1; pcb2] } in
  let manager_after = Manager.schedule_and_switch manager_ready in
  match manager_after.Manager.running with
  | Some p -> assert (p.Process.pid = 2) (* PID 2 tem periodo 15, menor que 20, logo maior prioridade em RM *)
  | None -> failwith "Erro no test_scheduler_rm"

let test_periodic_process () =
  let memory = Memory.create_memory () in
  memory.(0) <- Instructions.T;
  let manager = Manager.create_manager memory Manager.Priority in
  let pcb1 = Process.create_pcb 1 0 "prog" 0 1 1 0 (Some 50) in
  let manager_ready = { manager with Manager.ready_queue = [pcb1] } in
  let manager_after = Manager.command_E manager_ready in
  assert (List.length manager_after.Manager.terminated = 1);
  assert (List.length manager_after.Manager.new_queue = 1);
  let p_new = List.hd manager_after.Manager.new_queue in
  assert (p_new.Process.arrival_time = 50)

let test_command_T () =
  let memory = Memory.create_memory () in
  let manager = Manager.create_manager memory Manager.Priority in
  let pcb1 = Process.create_pcb 1 0 "p1" 0 1 1 0 None in
  let term_pcb = Process.terminate_process pcb1 10 in
  let manager_term = { manager with Manager.terminated = [term_pcb] } in
  let _ = Manager.command_T manager_term in
  (* Apenas certifica que não crasha ao imprimir estatísticas matemáticas *)
  ()

let test_arrivals () =
  let memory = Memory.create_memory () in
  let manager = Manager.create_manager memory Manager.Priority in
  let pcb1 = Process.create_pcb 1 0 "p1" 0 10 1 5 None in
